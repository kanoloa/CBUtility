...
const cb_server = {
    "baseUrl": env.parsed.CB_BASE,
    "username": env.parsed.CB_USER,
    "password": env.parsed.CB_PASS,
    "proxy": env.parsed.PROXY_URL
};
...
exports.setProxy = (proxy) => cb_server.proxy = proxy;
...
axios.interceptors.request.use(
    (config) => {
        ...
        if (cb_server.proxy) {
            config.httpsAgent = new HttpsProxyAgent(cb_server.proxy);
        }
        return config;
    }
)
